				...
				{currentlyPlaying.isLoading ? (
					...
					<HStack spacing={6}>
						<Image
							...
							alt="Album"
							src={
								currentlyPlaying.data?.data.item.album.images[0]
									.url
							}
						/>
						<Box>
							...
							>
								<Link
									href={
										currentlyPlaying.data.data.item
						...
						<Text>{`${Math.ceil(
							currentlyPlaying.data.data.progress_ms / 1000 / 60
						)}:${
							Math.floor(
								(currentlyPlaying.data.data.progress_ms /
									1000) %
									60
							) < 10
								? "0" +
